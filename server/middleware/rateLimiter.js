...
const apiLimiter = rateLimit({
  ...
  // keyGenerator: (req) => req.ip, // IPv6 호환 IP 기반 제한 (필요시 사용)
...
const authLimiter = rateLimit({
  ...
  skipFailedRequests: false,     // 실패 요청은 카운트
  skip: (req) => {
    // 개발 환경에서는 localhost(127.0.0.1)는 제한 제외
...
const sensitiveActionLimiter = rateLimit({
  ...
  message: {
    success: false,
    message:
      '중요한 작업에 대한 요청 한도를 초과했습니다. 1시간 후에 다시 시도해주세요.',
