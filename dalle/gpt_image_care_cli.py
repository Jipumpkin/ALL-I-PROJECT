...
# -*- coding: utf-8 -*-
"""
GPT-Image-1 기반 Node.js 백엔드 연동용 CLI 스크립트
케어 이미지 합성을 위한 커맨드라인 인터페이스

Usage:
python gpt_image_care_cli.py --animal <path> --space <path> --activity <activity> --output <path>
...
from gpt_image_synthesizer import GPTImageCareImageSynthesizer, get_supported_activities
import traceback
...
def main():
    parser = argparse.ArgumentParser(description='GPT-Image-1 케어 이미지 합성 CLI')
    ...
    parser.add_argument('--activity', required=True,
                       choices=['밥주기', '씻기기', '미용하기', 'feeding', 'bathing', 'grooming'],
                       help='케어 활동 선택')
    ...
    # 영어 키워드를 한글로 변환
    if args.activity in activity_mapping:
        args.activity = activity_mapping[args.activity]
    ...
    try:
        ...
        # 출력 메시지 제어
        if not args.quiet:
            print(f"🚀 GPT-Image-1 케어 이미지 합성 시작: {args.activity}", file=sys.stderr)
        ...
        if args.format == 'json':
            ...
            # 텍스트 형태로 출력
            if result["success"]:
                print(f"✅ GPT-Image-1 합성 성공!")
                print(f"📁 파일: {result['filename']}")
                print(f"📍 경로: {result['image_path']}")
                print(f"🤖 모델: {result.get('model', 'gpt-image-1')}")
            else:
